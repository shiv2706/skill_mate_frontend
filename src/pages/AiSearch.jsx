    ...
    const AiFilter = async()=> {
        try{
            if(formData.skills.length > 0){
                ...
                setSearching(true)
                const FilteredProfiles = await axios.post('/smartfilter/filter-profiles', {userInput:formData.skills, allProfiles: allProfiles},{withCredentials: true});
                console.log("all profiles are: " + FilteredProfiles.data);
                console.log("names are: " + FilteredProfiles.data[0])
                const results = await axios.post('/profile/get-profiles-name',{names:FilteredProfiles.data},{withCredentials: true});
                console.log("names are: " + JSON.stringify(results.data))
                setDisplayProfile(results.data.data)
                setSearching(false)
                setDataRecieved(true)
            }else{
                setSearching(false)
            ...
            <header className="absolute border-b border-gray-300 inset-x-0 top-0 z-50 ">
                <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
                    ...
                    <div className="hidden lg:flex lg:gap-x-12">
                        ...
                        <button
                            className="sm:text-xl font-semibold text-gray-900 transition-transform duration-300 hover:scale-110  cursor-pointer" onClick={PeopleHandler}>
                            People
                        </button>
                        <button
                            ...
                            <div className="-my-6 divide-y divide-gray-500/10">
                                ...
                                <div className="space-y-2 py-6">
                                    <button onClick={ProfileHandler}
                                            className="sm:text-xl font-semibold text-gray-900 cursor-pointer transition-transform duration-300 hover:text-indigo-500 hover:scale-110 ">
                                        My Profile
                                    </button>
                                </div>
                                <div className="space-y-2 py-6">
                ...
                <div className="relative pt-14">
                    ...
                    <div className="py-24 sm:py-15">
                        <div className="mx-auto max-w-7xl px-6 lg:px-8">
                            <h1 className="flex text-4xl text-center mb- justify-center font-serif  text-gray-900 sm:text-6xl">
                                AI Search <SparklesIcon aria-hidden="true"
                                                        className="-mr-1 size-5 text-indigo-500"/>
                            </h1>
                            <p className="mt-5 mb-10 text-lg font-medium text-pretty text-center text-gray-500 sm:text-xl/8">
                                Find you SKILLMATE faster with AI Search. No need for countless hours of scrolling, just
                                enter the required skills or Keywords.
                            </p>
                            <div
                                className="md:flex md:items-center pt-3 pb-3 pl-3 pr-3 w-full rounded-lg bg-transparent shadow-sm md:justify-between">
                                <div>
                                    <input
                                        name="skills"
                                        type="text"
                                ...
                                <div className="mt-4 flex md:mt-0 md:ml-4">
                                    <button onClick={AiFilter}
                                        className=" inline-flex cursor-pointer items-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600"
                                    >
                                        Search
                                    </button>
                                </div>
                            </div>
                            {!dataRecieved && !searching && <div className="text-center mt-10">
                                ...
                                <p className="mt-1 text-sm text-gray-500">Get started by entering skills</p>

                            </div>}
                            {dataRecieved && <ul role="list"
                                                 className="grid grid-cols-1 mt-10 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                                {displayProfile.map((person) => (
                                    <li
                                        key={person.email}
                                        className="col-span-1 flex transition-transform duration-300 hover:scale-102 flex-col divide-y divide-gray-200 rounded-lg bg-white text-center shadow-sm"
                                    >
                                        <div className="flex flex-1  flex-col p-4">
                                            ...
                                            <h3 className="mt-6 text-sm font-medium text-gray-900">Top Skills</h3>
                                            <dl className="mt-1 flex flex-wrap flex-row justify-center justify-beetween gap-1 ">
                                        ...
                                        <div>
                                            <div className="-mt-px flex divide-x divide-gray-200">
                                                <div className="flex w-0 flex-1">
                                                    <Link to={`/profiledetails/${person.profileId}`}
                            ...
                            {searching && <div role="list"
                                                   className="grid grid-cols-1 mt-10 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                                <div
                                    className="col-span-1 flex h-85 transition-transform duration-300 hover:scale-102 flex-col divide-y divide-gray-200 rounded-lg bg-white text-center shadow-sm">
                                    <Stack spacing={1} className="justify-items-center mt-4">
